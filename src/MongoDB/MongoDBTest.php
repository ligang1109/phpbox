...
class MongoDBTest extends TestCase
{
    ...
    public function testUpdate($dao)
    {
        ...
        $dao->insertOne($item);
        $dao->updateById("$id", array('name' => Driver::genObjectId(), 'status' => 2));
     ...
     * @param $dao MiscDao
     */
    public function testFindByIdFindByIds($dao)
    {
        $item  = array(
            '_id'  => Driver::genObjectId(),
            'name' => 'tfid',
        ...
        $insertR = $dao->insertOne($item);
        $id      = $insertR->getInsertedId();
        $ids[]   = $id;
        $findR   = $dao->findById($id);
        $this->assertEquals($insertR->getInsertedId(), $findR['_id']);

        $data = $dao->findByIds($ids);
        foreach ($data as $i => $item) {
            $this->assertEquals($item['_id'], $ids[$i]);
        }
    }
}